...
static int xSemaphoreTake(int *m, unsigned ticks) {
    ...
    if (ticks == portMAX_DELAY) assert(!atomic_load(&s_connection));
    if (locked) return 0;
...
static void clear_subscriptions(void) {atomic_store(&s_video_subscribed, false);}
...
with tempfile.TemporaryDirectory(prefix="s3-h5-video-") as tmp:
    path = Path(tmp)
    (path / "test.c").write_text(prefix + function("starter_tirtc_video_ready") +
