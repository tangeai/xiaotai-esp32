...
def function(path, name):
    ...
    match = re.search(r"(?m)^static (?:void|bool) " + re.escape(name) + r"\(", text)
    assert match, name
...
static uint32_t s_connection_generation=3;
...
with tempfile.TemporaryDirectory(prefix="ai-start-") as temp:
    ...
    subprocess.run([os.environ.get("CC", "gcc"), "-std=c11", "-Wall", "-Wextra", "-Werror",
                    *sanitizers, "-I", str(cjson),
                    str(directory / "test.c"), str(cjson / "cJSON.c"),
                    "-lm", "-o", str(directory / ("test.exe" if os.name == "nt" else "test"))], check=True)
    subprocess.run([str(directory / ("test.exe" if os.name == "nt" else "test"))], check=True, timeout=10)


connection = function(source, "handle_connection")
assert connection.index("if (!current)") < connection.index("send_ai_start();")
assert "AI_START_SETTLE_MS" not in (root / source).read_text(encoding="utf-8")
assert "s_ai_start_at_ms" not in (root / source).read_text(encoding="utf-8")
runtime = (root / source).read_text(encoding="utf-8")
assert "suspend_mqtt" not in runtime and "resume_mqtt" not in runtime
print("PASS: connected-generation gate retained, no settle timer or MQTT pause")
