...
def realtime_connect_memory(c):
    ...
    require(sum(bool(re.search(r"if \(!prepare_external_connect\(\)\)", s)) for s in lines) == 3,
            "every TiRTC external connect must release its reserve first")
    c.need("runtime", "VOIP_CONNECT_TASK_STACK_BYTES (24U * 1024U)",
           "xTaskCreateWithCaps(voip_connect_task", "restore_external_connect_reserve();",
    ...
    c.forbid("runtime", r"suspend_mqtt|resume_mqtt|esp_mqtt_client_(stop|destroy)")
    c.forbid("platform", r"suspend_mqtt_for_realtime|resume_mqtt_after_realtime")
    c.need("components/starter_runtime/src/starter_room.inc", "prepare_external_connect()",
