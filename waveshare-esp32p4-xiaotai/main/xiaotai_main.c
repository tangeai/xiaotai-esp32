...
/*
 * 启动任务仍保留已验证的内部栈；NVS 写入已交给 nvs_store，不能再将 NVS
 * 作为其他任务必须使用内部栈的理由。平台上线后的 HTTP 循环使用 PSRAM 栈。
 */
...
static void starter_start_task(void *argument)
{
    ...
    for (;;) {
         ...
         * gives it back immediately before an external call, keeping MQTT online.
         */
