...
static void finish_sync_request(ctrl_cmd_t *app_req, bool tx_done);
...
fail_req:
	...
	if (app_req->rpc_rsp_cb) {
		...
		if (post_sync_resp_sem(app_resp) != SUCCESS) {
...
static int process_rpc_rx_msg(Rpc * proto_msg, rpc_rx_ind_t rpc_rx_func)
{
	...
	/* 3. Check if it is response msg */
		...
		if (CALLBACK_AVAILABLE ==
			...
			/* A response belongs to its UID, not the next FIFO consumer. */
			if (post_sync_resp_sem(app_resp) != SUCCESS) {
				goto free_buffers;
			}
		}
...
static void rpc_rx_thread(void const *arg)
{
	...
	if (!sync_rsp_mutex) {
		ESP_LOGE(TAG, "RPC response registry is not created");
		return;
	}
...
static void rpc_tx_thread(void const *arg)
{
	...
	while (1) {
		...
		if (app_req) {
			bool synchronous = !app_req->rpc_rsp_cb;
			process_rpc_tx_msg(app_req);
			if (synchronous) {
				finish_sync_request(app_req, true);
			}
		} else {
...
static ctrl_cmd_t * get_response(int *read_len, ctrl_cmd_t *app_req)
{
	...
	*read_len = 0;
	int ret = wait_for_sync_response(app_req);
	ctrl_cmd_t *response = NULL;
	g_h.funcs->_h_lock_mutex(sync_rsp_mutex, HOSTED_BLOCK_MAX);
	for (int i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		if (sync_rsp_table[i].request == app_req) {
			response = sync_rsp_table[i].response;
			sync_rsp_table[i].response = NULL;
			break;
		}
	}
	g_h.funcs->_h_unlock_mutex(sync_rsp_mutex);
	if (response) {
		*read_len = sizeof(*response);
	} else {
...
static int post_sync_resp_sem(ctrl_cmd_t *app_resp)
{
	...
	g_h.funcs->_h_lock_mutex(sync_rsp_mutex, HOSTED_BLOCK_MAX);
	for (int i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		sync_rsp_t *slot = &sync_rsp_table[i];
		if (slot->uid != app_resp->uid || !slot->waiting || !slot->sem)
			continue;
		if (slot->response_id != app_resp->msg_id) {
			ESP_LOGE(TAG, "RPC response mismatch: uid=%lu expected=0x%lx got=0x%x",
				(unsigned long)app_resp->uid, (unsigned long)slot->response_id,
				app_resp->msg_id);
			ret = MSG_ID_OUT_OF_ORDER;
		} else if (!slot->response) {
			slot->response = app_resp;
			ret = g_h.funcs->_h_post_semaphore(slot->sem);
			if (ret != SUCCESS)
				slot->response = NULL;
		}
		break;
...
{
	int exp_resp_msg_id = (app_req->msg_id - RPC_ID__Req_Base + RPC_ID__Resp_Base);
	if (exp_resp_msg_id <= RPC_ID__Resp_Base || exp_resp_msg_id >= RPC_ID__Resp_Max)
		return MSG_ID_OUT_OF_ORDER;
	...
	g_h.funcs->_h_lock_mutex(sync_rsp_mutex, HOSTED_BLOCK_MAX);
	for (int i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		if (!sync_rsp_table[i].request) {
			sync_rsp_table[i] = (sync_rsp_t) {
				.uid = app_req->uid, .sem = app_req->rx_sem,
				.response_id = exp_resp_msg_id, .request = app_req, .waiting = true,
			};
	...
	if (ret != CALLBACK_SET_SUCCESS) {
		...
		app_req->rx_sem = NULL;
		ESP_LOGE(TAG, "Sync RPC registry full");
	}
	return ret;
}
...
static int wait_for_sync_response(ctrl_cmd_t *app_req)
{
	int timeout_sec = app_req->rsp_timeout_sec ? app_req->rsp_timeout_sec : DEFAULT_RPC_RSP_TIMEOUT;
	return g_h.funcs->_h_get_semaphore(app_req->rx_sem, timeout_sec);
}

/* The waiter and TX worker share the request until BOTH are done. Timeout must
 * not free a queued/in-flight request or a semaphore still visible to RX. */
static void finish_sync_request(ctrl_cmd_t *app_req, bool tx_done)
{
	ctrl_cmd_t *free_req = NULL;
	ctrl_cmd_t *free_resp = NULL;
	void *free_sem = NULL;
	g_h.funcs->_h_lock_mutex(sync_rsp_mutex, HOSTED_BLOCK_MAX);
	for (int i = 0; i < MAX_SYNC_RPC_TRANSACTIONS; i++) {
		sync_rsp_t *slot = &sync_rsp_table[i];
		if (slot->request != app_req)
			continue;
		if (tx_done) {
			slot->tx_done = true;
		} else {
			...
			free_sem = slot->sem;
			slot->sem = NULL;
			free_resp = slot->response;
			slot->response = NULL;
		}
		if (slot->tx_done && !slot->waiting) {
			free_req = slot->request;
			*slot = (sync_rsp_t) {0};
		}
		break;
	}
	g_h.funcs->_h_unlock_mutex(sync_rsp_mutex);
	if (free_sem)
		g_h.funcs->_h_destroy_semaphore(free_sem);
	CLEANUP_APP_MSG(free_resp);
	CLEANUP_APP_MSG(free_req);
}
...
ctrl_cmd_t * rpc_wait_and_parse_sync_resp(ctrl_cmd_t *app_req)
{
	...
	finish_sync_request(app_req, false);
	return rx_buf;
}
...
int rpc_core_deinit(void)
{
	...
	if (sync_rsp_mutex) {
		g_h.funcs->_h_destroy_mutex(sync_rsp_mutex);
		sync_rsp_mutex = NULL;
	}
...
int rpc_core_init(void)
{
	...
	sync_rsp_mutex = g_h.funcs->_h_create_mutex();
	if (!sync_rsp_mutex)
		goto free_bufs;
