...
static atomic_uchar transport_state = TRANSPORT_INACTIVE;
static atomic_int transport_init_error;
static atomic_bool failed_init_cleaned;

void transport_drv_report_init_error(esp_err_t error)
{
	int expected = ESP_OK;
	if (error != ESP_OK)
		atomic_compare_exchange_strong(&transport_init_error, &expected, error);
}

static esp_err_t transport_check_init_error(void)
{
	esp_err_t error = atomic_load(&transport_init_error);
	if (error != ESP_OK && !atomic_exchange(&failed_init_cleaned, true)) {
		/* Only the pre-data-path failure is handled here. Do not tear down
		 * a live transport or silently restart a failed card. */
		transport_state = TRANSPORT_INACTIVE;
#if H_TRANSPORT_IN_USE == H_TRANSPORT_SDIO
		transport_cleanup_failed_init();
#endif
		ESP_LOGE(TAG, "transport initialization failed: %s", esp_err_to_name(error));
	}
	return error;
}
...
{
	atomic_store(&transport_init_error, ESP_OK);
	atomic_store(&failed_init_cleaned, false);
...
esp_err_t transport_drv_reconfigure(void)
{
	...
	esp_err_t error = transport_check_init_error();
	...
	if (!is_transport_tx_ready()) {
		...
		while (!is_transport_tx_ready()) {
			error = transport_check_init_error();
			if (error != ESP_OK) return error;
