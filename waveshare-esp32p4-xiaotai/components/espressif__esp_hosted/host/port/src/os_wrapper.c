...
void *hosted_thread_create(char *tname, uint32_t tprio, uint32_t tstack_size, void (*start_routine)(void const *), void *sr_arg)
{
	...
	hosted_thread_t *thread_handle = hosted_malloc(sizeof(*thread_handle));
	if (!thread_handle) {
		ESP_LOGE(TAG, "Failed to allocate thread handle\n");
		return NULL;
	}
	thread_handle->task = NULL;
	thread_handle->with_caps = true;
	...
	*thread_handle = osThreadCreate(osThread(Ctrl_port_tsk), arg);
	...
	task_created = xTaskCreateWithCaps((void (*)(void *))start_routine,
			...
			&thread_handle->task,
			stack_caps);
	if (task_created != pdTRUE || !thread_handle->task) {
		thread_handle->task = NULL;
		thread_handle->with_caps = false;
...
int hosted_thread_cancel(void *thread_handle)
{
	...
	hosted_thread_t *thread = thread_handle;
	TaskHandle_t task = thread->task;
	bool with_caps = thread->with_caps;
	/* WithCaps creates static TCB/stack allocations that vTaskDelete alone
	 * never frees. Retain the creation kind across the internal-RAM fallback. */
	HOSTED_FREE(thread_handle);
	if (with_caps) vTaskDeleteWithCaps(task);
	else vTaskDelete(task);
	return RET_OK;
}
