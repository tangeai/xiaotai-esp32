 ...
 * @brief H5、AI、呼叫和多人对讲的媒体会话状态机。
 *
 * 本模块隐藏 token 获取、WHIP 建连、AI JSON-RPC 握手、超时和迟到回调过滤。
 * 业务状态由一个 FreeRTOS 任务持有；调用者投递意图并读取快照。
 * 按住讲话的音频门禁为原子操作，松手不等待业务队列。
 */
...
typedef enum {
    STARTER_ROOM_EMPTY = 0, STARTER_ROOM_SYNCING, STARTER_ROOM_CONNECTING,
    STARTER_ROOM_LISTENING, STARTER_ROOM_SUSPENDED, STARTER_ROOM_RETRY,
    STARTER_ROOM_LEAVING, STARTER_ROOM_ERROR,
} starter_room_state_t;
typedef struct {
    char participant_id[129];
    char device_id[65];
    char name[65];
    bool self;
    uint8_t mic; /**< 0 off, 1 on, 2 speaking. */
} starter_room_member_t;
typedef struct {
    uint32_t revision;
    uint32_t connection_generation;
    starter_room_state_t state;
    char room_code[7];
    char self_name[65]; /**< Server device_name; empty means use device_id. */
    char message[97];
...
} starter_room_snapshot_t;

/** Nonblocking UI intents. Codes/passwords are digit strings, not integers. */
/** Close suspends this app's connection, not its durable room assignment. */
void starter_runtime_room_set_open(bool open);
esp_err_t starter_runtime_room_create(const char *password);
esp_err_t starter_runtime_room_join(const char *code, const char *password);
esp_err_t starter_runtime_room_leave(void);
esp_err_t starter_runtime_room_refresh(void);
/** Call on PRESSING as well as PRESSED; release/deletion closes the gate directly. */
bool starter_runtime_room_ptt(uint32_t generation, bool pressed);
/** Small paged copy. Caller should keep the buffer in PSRAM. */
 ...
 * 外连前只释放该块，永久 MQTT 保持在线；调用方仅在启动编排阶段调用一次。
 */
...
/* Nonblocking UI read. On contention, leave out unchanged and try next tick. */
