...
void starter_tirtc_room_set_transmitting(uint32_t generation, bool enabled)
{
    if (enabled) {
        if (generation != 0 && starter_tirtc_generation() == generation &&
            starter_tirtc_mode() == STARTER_TIRTC_ROOM) {
            atomic_store(&s_room_tx_generation, generation);
        }
    } else {
        unsigned expected = generation;
        (void)atomic_compare_exchange_strong(&s_room_tx_generation, &expected, 0);
    }
}

bool starter_tirtc_room_transmitting(uint32_t generation)
{
    return generation != 0 && atomic_load(&s_room_tx_generation) == generation;
}
