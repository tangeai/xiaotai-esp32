 ...
 * @brief 产品音频硬件的唯一接入点。
 *
 * 本工程使用微雪 ESP32-P4 的 ES8311 单麦与 DAC 回采参考。
 * 协议状态机和 TiRTC SDK 适配层不感知板级细节；一个
...
/** ROOM-only PTT. Never changes the user's global microphone preference. */
...
typedef struct {
    ...
    uint8_t speaker_volume_applied; /**< Last successfully applied codec volume. */
    bool speaker_muted_applied; /**< Last successfully applied codec mute. */
    esp_err_t speaker_control_error; /**< Actual codec/PA result, not NVS status. */
...
/** Nonblocking speaker requests. ESP_OK means accepted, not codec I/O complete.
 * The existing output owner applies the latest request at a PCM block boundary
 * (or while idle). Check status applied fields/control_error for actual result.
 * No NVS operation, per-request allocation, or extra task is created here. */
