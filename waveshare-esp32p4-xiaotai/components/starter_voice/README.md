...
模型更换后，在 Linux/WSL 中使用 C++17 编译器运行 `python3 -B tools/test_wake_model.py`，检查模型结构、算子、量化与分类头，并核对上述文件摘要。设备启动时应出现 `KWS nhwc-20260915` 和 `wake self-test=ESP_OK`；随后分别测试快、正常、慢语速，记录命中次数与推理耗时。
